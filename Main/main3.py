...
def train():
  ...
  model.fit(training_x, training_y, batch_size = 32, epochs=100, verbose=2)
...
predict_y = pd.DataFrame(model.predict(training_x), columns = ['price'])
plot_train = pd.concat([training_x, training_y], axis = 1)
plot_predict = pd.concat([training_x, predict_y], axis = 1)

# print(plot_train.loc[[0, 1]])
# print(plot_predict.loc[[0, 1]])


plot_predict['Over/Undervalued (blue means undervalued)'] = plot_predict['price']//plot_train['price']
# print(plot_predict.loc[[0, 1]])
